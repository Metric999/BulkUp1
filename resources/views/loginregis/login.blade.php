...
<body class="flex h-screen font-[Poppins]">
  ...
  <div class="w-1/2 bg-gray-400 flex flex-col justify-center items-center px-10">
    ...
    <form class="w-full max-w-md flex flex-col" method="POST" action="{{ route('login') }}"> @csrf
        ...
        <button type="submit" class="bg-white text-black px-6 py-2 rounded font-semibold hover:bg-indigo-500 transition">LOGIN</button>
            <a href="{{ route('register') }}" class="bg-white text-black px-6 py-2 rounded font-semibold hover:bg-indigo-500 transition">REGISTER</a>
        </div>
    </form>
  </div>
</body>
